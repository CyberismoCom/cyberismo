...
program
    ...
    .argument('[cardkey]', 'Cardkey of card; if omitted "calc generate" affects the whole cardtree. "calc run" always needs "card key"')
    .option('-p, --project-path [path]', `${pathGuideline}`)
...
create
    ...
    .argument('<prefix>', 'Prefix that will be part of each card\'s cardkey. Prefix can be 3-10 characters (A-Z)')
...
program
    ...
    .argument('<type>', `Possible types: ${Commands.removableTypes.join(', ')}`, parseRemovableTypes)
    .argument('<targetName>', 'Resource that should be removed; template name or cardkey. If removing an attachment, define cardkey of owning card.')
...
program
    ...
    .argument('<type>', 'resource types: attachments, card, cards, cardtype, cardtypes, project, template, templates, workflow, workflows', parseSupportedTypes)
    .argument('[typeDetail]', 'additional information about the requested type; for example a cardkey')
    .option('-d --details', 'Certain resources (such as cards) can have additional details')
    .option('-p, --project-path [path]', `${pathGuideline}`)
