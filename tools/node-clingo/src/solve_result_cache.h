...
namespace node_clingo
{
    ...
    struct ClingoSolveException : std::runtime_error
    {
        ...
        ClingoSolveException(const std::string& what, std::vector<ClingoLogMessage> logs, std::string programKey = "")
            : std::runtime_error(what), logs(std::move(logs)), programKey(std::move(programKey))
        {
        }
    };
