...
export const ContentArea: React.FC<ContentAreaProps> = ({
  ...
  const handleDownload = (htmlElementWithSVG: HTMLElement) => {
    ...
    a.href = url;
    a.download = (svgEl.id || 'Diagram') + '.svg';
  ...
  useEffect(() => {
    ...
    wrappers.forEach((wrapper) => {
      ...
      (wrapper as any).__cleanupSvgControls = cleanup;
    });

    /* ---------- Global cleanup on effect teardown ---------- */
    return () => {
      wrappers.forEach((wrapper) => {
        (wrapper as any).__cleanupSvgControls?.();
        delete (wrapper as any).__cleanupSvgControls;
      });
    };
