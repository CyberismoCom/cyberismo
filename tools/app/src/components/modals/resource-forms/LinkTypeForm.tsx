...
export function LinkTypeForm({
  ...
  const onFormSubmit = async (data: CreateLinkTypeData) => {
    await onSubmit(data);
  };
      ...
      <Stack spacing={2}>
          ...
          render={({ field }) => (
            <FormControl error={!!errors.identifier}>
              ...
              <Input
                ...
                startDecorator={<ProjectIdentifier type="linkTypes" />}
              />
        ...
        >
          {t('newResourceModal.create', {
            resourceType: t('newResourceModal.linkTypes.name'),
          })}
        </Button>
      </Stack>
    </form>
