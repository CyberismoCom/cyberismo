...
export function GraphModelForm({
  onSubmit,
  isSubmitting,
}: ResourceFormProps<CreateGraphModelData>) {
  const { t } = useTranslation();
  const {
    control,
    handleSubmit,
    formState: { errors, isValid },
  } = useForm<CreateGraphModelData>({
    mode: 'onChange',
    defaultValues: {
      identifier: '',
    },
  ...
  const onFormSubmit = async (data: CreateGraphModelData) => {
    await onSubmit(data);
  };
      ...
      <Stack spacing={2}>
          ...
          render={({ field }) => (
            <FormControl error={!!errors.identifier}>
              ...
              <Input
                ...
                startDecorator={<ProjectIdentifier type="graphModels" />}
              />
        ...
        >
          {t('newResourceModal.create', {
            resourceType: t('newResourceModal.graphModels.name'),
          })}
        </Button>
      </Stack>
    </form>
