 ...
 *         otherRoot (card)
 *       templates: test/templates/empty (no cards)
 *     eucra module (readOnly: true)
 *       templates: eucra/templates/page
 *         moduleRoot (card)
 *       templates: eucra/templates/empty (no cards, readOnly)
 */
      ...
      children: [
        moduleNode(
          ...
          [
            ...
            templatesNode('test/templates/empty', []),
          ],
          false,
        ),
        moduleNode(
          ...
          [
            ...
            templatesNode('eucra/templates/empty', [], true),
          ],
          true,
        ),
      ],
...
const emptyTemplateId = 'templates-test/templates/empty';
const moduleTemplateId = 'templates-eucra/templates/page';
const moduleEmptyTemplateId = 'templates-eucra/templates/empty';
...
describe('resolveConfigTreeMove', () => {
  ...
  test('drop on an empty local templates node emits root:<name> sentinel', () => {
    const tree = buildFixture();
    expect(resolveConfigTreeMove(tree, 'root1', emptyTemplateId, 0)).toEqual({
      kind: 'update',
      cardKey: 'root1',
      update: { parent: 'root:test/templates/empty', index: 0 },
    });
  });

  test('drop on an empty read-only templates node is rejected as moduleReadOnly', () => {
    const tree = buildFixture();
    expect(
      resolveConfigTreeMove(tree, 'root1', moduleEmptyTemplateId, 0),
    ).toEqual({
      kind: 'reject',
      reason: 'moduleReadOnly',
    });
