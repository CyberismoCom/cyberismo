...
describe('move command', () => {
  ...
  // The tree refuses the same move on its own (see 'CardTree structural
  // integrity' in card-tree.test.ts) - a cycle makes derived paths
  // non-terminating, so that layer is an integrity invariant, not a message.
  ...
  it('rankByIndex(last) at template root does not throw cross-parent', async () => {
    const moveResult = await commandHandler.command(
      Cmd.move,
      ['decision_1', 'root:decision/templates/simplepage'],
      options,
    );
    ...
    await expect(
      commandManager.moveCmd.rankByIndex('decision_1', 2),
