...
describe('move command', () => {
  ...
  it('completes the move on a retry after the rank write fails', async () => {
    ...
    const tree = project.cardTree as unknown as {
      writeRank: (cardKey: string, rank: string) => Promise<void>;
    };
    const writeRank = tree.writeRank.bind(tree);
    let failRankWrite = true;
    tree.writeRank = async (cardKey, rank) => {
      if (failRankWrite) {
        failRankWrite = false;
        throw new Error('rank write failed');
      }
      return writeRank(cardKey, rank);
    };
