...
describe('resources', function () {
  ...
  describe('resource-cache', () => {
    it('collect local resources', async () => {
      // Resources should be automatically collected on Project initialization
      const calcCount = project.calculations().length;
      const cardTypesCount = project.cardTypes().length;
      const fieldTypesCount = project.fieldTypes().length;
      const graphModelCount = project.graphModels().length;
      const graphViewCount = project.graphViews().length;
      const linkTypesCount = project.linkTypes().length;
      const reportsCount = project.reports().length;
      const templatesCount = project.templates().length;
    ...
    it('resource existence checks', () => {
      ...
      expect(project.resourceExists(testWorkflow)).to.equal(true);
      expect(project.resourceExists(testCardType)).to.equal(true);
      expect(project.resourceExists(testFieldType)).to.equal(true);

      // Test non-existent resource
      expect(project.resourceExists('nonexistent')).to.equal(false);
    });
  });
  ...
  describe('resource basic operations', () => {
    ...
    resourceConfigs.forEach((config) => {
      it(`create ${config.type}`, async () => {
        ...
        if (config.type === 'cardTypes') {
          ...
          const after = project.fieldTypes();
          found = after.find((item) => item.data?.name === res.data?.name);
        } else if (config.type === 'reports') {
          const res = project.resourceByType(name, config.type);
          await res.createReport();
          const after = project.reports();
          found = after.find((item) => item.data?.name === res.data?.name);
        } else {
          const res = project.resourceByType(name, config.type);
          await res.create();
          const after = project[config.type]();
          found = after.find((item) => item.data?.name === res.data?.name);
        }
    ...
    it('create link type with provided content', async () => {
      ...
      const before = project.linkTypes();
      let found = before.find((item) => item.data?.name === name);
      expect(found).to.equal(undefined);
      const linkTypeData = {
        name: name,
      ...
      const res = project.resourceByType(name, 'linkTypes');
      await res.create(linkTypeData);
      const after = project.linkTypes();
    ...
    it('create template with provided content', async () => {
      ...
      const before = project.templates();
      let found = before.find((item) => item.data?.name === name);
      expect(found).to.equal(undefined);
      const templateData = {
        name: name,
      ...
      const res = project.resourceByType(name, 'templates');
      await res.create(templateData);
      const after = project.templates();
      found = after.find((item) => item.data?.name === name);
      expect(found).to.not.equal(undefined);
    });
    ...
    it('create workflow with provided content', async () => {
      ...
      const before = project.workflows();
      let found = before.find((item) => item.data?.name === name);
      expect(found).to.equal(undefined);
      const workflowData = {
        name: name,
      ...
      const res = project.resourceByType(name, 'workflows');
      await res.create(workflowData);
      const after = project.workflows();
      found = after.find((item) => item.data?.name === name);
      expect(found).to.not.equal(undefined);
    });
    it('create calculation with provided content', async () => {
      ...
      const before = project.calculations();
      let found = before.find((item) => item.data?.name === name);
      expect(found).to.equal(undefined);
      ...
      const res = project.resourceByType(name, 'calculations');
      await res.create(calculationData);
      const after = project.calculations();
      found = after.find((item) => item.data?.name === name);
      expect(found).to.not.equal(undefined);
    });
    ...
    it('try to create card type with invalid project prefix', async () => {
      ...
      const res = project.resourceByType(name, 'cardTypes');
      await expect(
    ...
    it('try to create field type with invalid project prefix', async () => {
      ...
      const res = project.resourceByType(name, 'fieldTypes');
    ...
    it('try to create resources with invalid project prefix', async () => {
      const resources = [
        project.resourceByType('unknown/calculations/new-one', 'calculations'),
        project.resourceByType('unknown/graphModels/new-one', 'graphModels'),
        project.resourceByType('unknown/graphViews/new-one', 'graphViews'),
        project.resourceByType('unknown/linkTypes/new-one', 'linkTypes'),
        project.resourceByType('unknown/templates/new-one', 'templates'),
        project.resourceByType('unknown/workflows/new-one', 'workflows'),
      ];
    ...
    it('try to create report with invalid project prefix', async () => {
      ...
      const res = project.resourceByType(name, 'reports');
    ...
    it('show graph model', async () => {
      ...
      const res = project.resourceByType(name, 'graphModels');
      const data = await res.show();
    ...
    it('show graph view', async () => {
      ...
      const res = project.resourceByType(name, 'graphViews');
      const data = await res.show();
      expect(data).to.deep.equal({
        content: {
          schema: {
    ...
    it('try to validate missing resource types', async () => {
      const resources = [
        ...
        project.resourceByType('unknown/cardTypes/not-exist', 'cardTypes'),
        project.resourceByType('unknown/fieldTypes/not-exist', 'fieldTypes'),
        project.resourceByType('unknown/graphModels/not-exist', 'graphModels'),
        project.resourceByType('unknown/graphViews/not-exist', 'graphViews'),
        project.resourceByType('unknown/linkTypes/not-exist', 'linkTypes'),
        project.resourceByType('unknown/reports/not-exist', 'reports'),
        project.resourceByType('unknown/templates/not-exist', 'templates'),
        project.resourceByType('decision/workflows/not-exist', 'workflows'),
      ];
      for (const resource of resources) {
    ...
    // Update tests (card type specific)
    ...
    it('update card type - add element to alwaysVisibleFields', async () => {
      ...
      const newFieldType = project.resourceByType(nameFT, 'fieldTypes');
    ...
    it('update card type - add two elements to alwaysVisibleFields and move the latter to first', async () => {
      ...
      const secondNewFieldType = project.resourceByType(nameFT, 'fieldTypes');
    ...
    it('update card type - add element to customFields', async () => {
      ...
      const fieldType = project.resourceByType(name, 'fieldTypes');
      if (!fieldType.data) {
        await fieldType.createFieldType('shortText');
      }

      const nameCT = 'decision/cardTypes/customFields';
      const res = project.resourceByType(nameCT, 'cardTypes');
      await res.createCardType('decision/workflows/decision');
    ...
    it('update card type - add two elements to customFields, then move last one to first', async () => {
      const name = 'decision/fieldTypes/newOne';
      const fieldType1 = project.resourceByType(name, 'fieldTypes');
      const name2 = 'decision/fieldTypes/secondNewOne';
      const fieldType2 = project.resourceByType(name2, 'fieldTypes');
    ...
    it('update existing workflow - rename state', async () => {
      ...
      const cardsWithThisWorkflow = cards.filter((card) => {
        const ct = project.resourceByType(
