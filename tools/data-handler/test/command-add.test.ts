...
describe('add command', () => {
  ...
  // A batch's keys and ranks are allocated in one pass, before any card is
  // written, and handed out zipped: the Nth key holds the Nth rank. Allocating
  // a rank per card instead let every card in the batch read the same sibling
  // snapshot, and so land on the same rank.
  it('gives a batch of added cards distinct, increasing ranks', async () => {
    ...
    // In the order the keys came back: a rank orders the cards, so the batch
    // is in increasing order or it is not the order the caller was handed.
