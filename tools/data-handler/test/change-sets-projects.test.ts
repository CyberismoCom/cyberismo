...
describe('changesets in a repository of several projects', () => {
  let dir: string;
  let repo: string;
  ...
  beforeEach(async () => {
    ...
    for (const name of ['one', 'two']) {
      await copyDir('test/test-data/valid/decision-records', join(repo, name));
    }
  ...
  it('takes over records kept for the whole repository before', async () => {
    ...
    expect((await later.list()).map((record) => record.id)).toEqual([info.id]);
