...
// ismo
import { readADocFileSync, readJsonFile, readJsonFileSync } from '../../src/utils/json.js';
...
describe('read Json files', () => {
    ...
    it('readJsonFile not found', async () => {
        const path = join(process.cwd(), 'test', 'test-data', 'i-do-not-exist.json');
        try {
    ...
    it('readJsonFile non-JSON content', () => {
        const path = join(process.cwd(), 'test', 'test-data', 'non-valid-template-not-json.txt');
        try {
            readJsonFileSync(path);
        } catch (error) {
            if (error instanceof Error) {
    ...
    it('formatJson', () => {
        const referenceJson = "{\n    \"summary\": \"Untitled\",\n    \"cardtype\": \"page\"\n}";
        const formattedJson = formatJson({ summary: "Untitled", cardtype: "page" });
