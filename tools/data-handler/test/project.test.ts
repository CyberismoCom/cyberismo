...
describe('project', () => {
    ...
    it('create class - resources (success)', async () => {
        ...
        if (workflows && workflows.length > 1) {
            ...
            for (const workflow of workflows) {
                if (workflow) {
                    const initialState = await project.workflowInitialState(workflow.name);
                    const expectedState = expectedInitialStates.at(workflows.indexOf(workflow));
    ...
    it('create settings class (success)', async () => {
        ...
        const configFile = join(decisionRecordsPath, '.cards', 'local', Project.projectConfigFileName);
    ...
    it('multiple instances of settings class', async () => {
        ...
        const configFile1 = join(decisionRecordsPath, '.cards', 'local', Project.projectConfigFileName);
        const configFile2 = join(emptyProjectPath, '.cards', 'local', Project.projectConfigFileName);
        ...
        expect(projectSettings1.cardkeyPrefix).to.not.equal(projectSettings2.cardkeyPrefix);
        expect(projectSettings2.cardkeyPrefix).to.equal(projectSettings3.cardkeyPrefix);
        expect(projectSettings1.nextAvailableCardNumber).to.not.equal(projectSettings2.nextAvailableCardNumber);
        expect(projectSettings2.nextAvailableCardNumber).to.equal(projectSettings3.nextAvailableCardNumber);
    ...
    it('access card details by id (success)', async () => {
        ...
        const card = await project.cardDetailsById(cardToOperateOn, { metadata: true });
        ...
        const additionalCardDetails = await project.cardDetailsById(cardToOperateOn, details);
        expect(additionalCardDetails).to.not.equal(undefined);
        if (additionalCardDetails) {
            expect(additionalCardDetails.metadata?.summary).to.equal('Decision Records');
            expect(additionalCardDetails.metadata?.cardtype).to.equal('simplepage-cardtype');
            expect(additionalCardDetails.metadata?.workflowState).to.equal('Created');
            expect(additionalCardDetails.children?.find(item => item.key === 'decision_6')).to.not.be.undefined;
    ...
    it('change card state (success)', async () => {
        ...
        const cardDetails = await project.cardDetailsById(cardToOperateOn, { metadata: true });
    ...
    it('find certain card from project - content as adoc (success)', async () => {
        ...
        const existingCard = await project.findSpecificCard('decision_5', { content: true, contentType: 'adoc' });
    ...
    it('find certain card from project - content as html (success)', async () => {
        ...
        const existingCard = await project.findSpecificCard('decision_5', { content: true, contentType: 'html' });
        expect(existingCard).to.not.equal(undefined);
    });
