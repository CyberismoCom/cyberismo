...
describe('project', () => {
  ...
  it('create class - resources (success)', async () => {
    ...
    const cardTypes = project.cardTypeNames();
    expect(cardTypes.length).to.equal(2);
    const cardType1 = project.resourceByType(
      cardTypes.at(0)!,
      'cardTypes',
    ).data;
    const cardType2 = project.resourceByType(
      cardTypes.at(1)!,
      'cardTypes',
    ).data;
    ...
    if (templates) {
      for (const template of templates) {
        if (template.data) {
          expect(project.resourceExists(template.data.name)).to.equal(true);
          const fetchTemplate = project.resourceByType(
            template.data.name,
            'templates',
          ).data;
    ...
    expect(project.resourceExists('idontexist')).to.equal(false);
  ...
  it('access card type details (success)', async () => {
    ...
    const cardTypeDetails = project.resourceByType(name, 'cardTypes').data;
  ...
  it('try to access card type details with non-existing name', async () => {
    ...
    const cardTypeDetails = project.resourceByType(
      'i-dont-exist',
      'cardTypes',
    ).data;
  ...
  it('access workflow details (success)', async () => {
    ...
    const workflowDetails = project.resourceByType(name, 'workflows').data;
  ...
  it('try to access workflow details with non-existing name', async () => {
    ...
    const workflowDetails = project.resourceByType(
      'i-dont-exist',
      'workflows',
    ).data;
  ...
  it('create template object from project (success)', async () => {
    ...
    const template = project
      .resourceByType('decision/templates/decision', 'templates')
      .templateObject();
  ...
  */
  ...
  it('should have template card attachments', async () => {
    const decisionRecordsPath = join(testDir, `valid${sep}decision-records`);
    const project = new Project(decisionRecordsPath);
    await project.populateCaches();
    const templateAttachments = (await project.attachmentsFromTemplates()).map(
      (item) => item.fileName,
    );
    // decision_2 should have an attachment
    expect(templateAttachments).to.include('the-needle.heic');
  });
