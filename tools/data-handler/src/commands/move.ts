...
export class Move {
  ...
  public async moveCard(source: string, destination: string) {
    ...
    if (sourceTree === targetTree && sourceCard.parent === newParent) {
      return;
    }
  ...
  public async rankCard(cardKey: string, beforeCardKey: string) {
    ...
    await tree.reorderAfter(cardKey, beforeCardKey);
  }
  ...
  public async rankFirst(cardKey: string) {
    await this.project.treeOf(cardKey).reorderFirst(cardKey);
  }
  ...
  public async rebalanceChildren(parentCardKey: string) {
    await this.project.treeOf(parentCardKey).rebalanceChildren(parentCardKey);
  }
