...
export class Validate {
  ...
  private async validateArrayOfFields(
    ...
    if (cardType && fieldArray) {
      const validationPromises = fieldArray.map(async (field) => {
        let fieldType;
        try {
          fieldType = project.resourceByType(field, 'fieldTypes').data;
        } catch {
          Validate.logger.info(`Cannot access field type '${field}'`);
        }
  ...
  public async validateCustomFields(
    ...
    let cardType;
    try {
      cardType = project.resourceByType(
        card.metadata?.cardType,
        'cardTypes',
      ).data;
    } catch {
      Validate.logger.info(`Cannot access metadata of card '${card.key}'`);
    }
    ...
    for (const field of cardType.customFields) {
      const found = project.resourceExists(field.name);
      if (!found) {
        validationErrors.push(
      ...
      let fieldType;
      try {
        fieldType = project.resourceByType(field.name, 'fieldTypes').data;
      } catch {
        Validate.logger.info(
          `Cannot access field type of card '${field.name}'`,
        );
      }
  ...
  public validateWorkflowState(project: Project, card: Card): string {
    ...
    try {
      cardType = card.metadata?.cardType
        ? project.resourceByType(card.metadata?.cardType, 'cardTypes').data
        : undefined;
    } catch {
      Validate.logger.info(`Cannot access card type of card '${card.key}'`);
    }
    if (!cardType) {
      validationErrors.push(
        `Card '${card.key}' has invalid card type '${card.metadata?.cardType}'`,
    ...
    let workflow;
    try {
      workflow = project.resourceByType(cardType.workflow, 'workflows').data;
    } catch {
      Validate.logger.info(
        `Cannot access workflow of card type '${cardType.name}' in card '${card.key}'`,
      );
    }
  ...
  public async validateResource(
    ...
    try {
      const resource = project.resourceByType(resourceName);
