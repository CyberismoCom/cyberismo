  ...
  public async migrate(
    ...
    if (toVersion !== undefined && toVersion !== SCHEMA_VERSION) {
      ...
      if (targetVersion !== currentVersion + 1) {
        throw new Error(
          `Cannot skip to version ${targetVersion}. Project is at version ${currentVersion}, next version is ${currentVersion + 1}. Migrate one version at a time with 'cyberismo migrate ${currentVersion + 1}', or use 'cyberismo migrate' to migrate to the latest version.`,
        );
      }
    }

    return await this.project.runMigrations(backupDir, timeoutMs);
  }
}
