...
export class CardContainer {
  ...
  private filterCardDetails(card: Card, details: FetchCardDetails): Card {
  ...
  private filterCardsDetails(cards: Card[], details: FetchCardDetails): Card[] {
  ...
  /**
   * Determines the location from a given path.
   ...
   */
  protected determineLocationFromPath(path: string): string {
    return cardPathParts(this.prefix, path).template || 'project';
  }

  /**
   * Populates the global card cache with all cards from all locations.
   */
  protected async populateCardsCache(): Promise<void> {}

  /**
   * Populates template cards into the global cache.
   */
   ...
   * @returns attachments from container.
   */
  protected attachments(path: string): CardAttachment[] {
    const attachments: CardAttachment[] = [];

    const targetLocation = this.determineLocationFromPath(path);
  ...
  public hasTemplateCard(cardKey: string): boolean {
    const cachedCard = this.cardCache.getCard(cardKey);
    if (cachedCard) {
      return cachedCard.location !== 'project';
    }
    return false;
  }
  ...
  protected showCards(path: string): Card[] {
    ...
    const relevantCards = Array.from(this.cardCache.getCards()).filter(
      (cachedCard) => cachedCard.location === targetLocation,
    );
   ...
   * @param cardKey The key of the card to remove
   * @return true, if card was removed, false otherwise
   */
  public async removeCard(cardKey: string): Promise<boolean> {
