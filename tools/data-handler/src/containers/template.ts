...
export class Template extends CardContainer {
  ...
  private async doCreateCards(
    ...
    const createMappingAndRanks = async () => {
      ...
      const futureSiblings = parentCard
        ? this.project.childCards(parentCard.children) || []
    ...
    try {
      ...
      const processedCards = await Promise.all(
        cards.map(async (originalCard) => {
          const card: Card = {
            key: originalCard.key,
            path: originalCard.path,
            content: originalCard.content,
            parent: originalCard.parent,
            children: [...(originalCard.children || [])],
  ...
  private findCardDirect(cardKey: string): Card | undefined {
  ...
  public cards(placeHolderPath?: string): Card[] {
    ...
    // Filter cards from the global cache that belong to this template
    // Use the full template name from the resource to match cache entries
   ...
   */
  public findCard(cardKey: string): Card | undefined {
