...
export class CardCache {
  ...
  // Builds the card cache from filesystem using recursive directory reading.
  ...
  /**
   * Removes card from cache.
   * @param cardKey card key to remove
   * @returns true, if card was removed from cache; false otherwise
   */
   ...
   * @returns true, if attachment was removed from cache; false otherwise
   */
  public deleteAttachment(cardKey: string, filename: string) {
    ...
    CardCache.logger.warn(
      `Cannot remove attachment '${filename}' for card '${cardKey}'`,
    );
    return false;
  }
  ...
  /**
   * Returns card from cache.
   * @param cardKey card key to find
   * @returns card from cache; if not found then returns undefined.
   */
   ...
   */
  public isValidTemplateNameFormat(templateName: string): boolean {
   ...
   * @param cards Cards to built the cache from.
   */
  public populateFromCards(cards: CachedCard[]) {
  ...
  public updateCard(cardKey: string, cardData: Card) {
    ...
    const extendedCard: CachedCard = {
      ...
      location: targetLocation,
      // Explicitly preserve these if they exist in the cached card but not in cardData
