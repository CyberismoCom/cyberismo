...
export class CalculationEngine {
  ...
  private async setCardTypesPrograms() {
    const cardTypes = this.project.cardTypes();
    for (const cardType of cardTypes) {
      if (cardType.data) {
        const cardTypeContent = createCardTypeFacts(cardType.data);
        setProgram(cardType.data.name, cardTypeContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setFieldTypesPrograms() {
    const fieldTypes = this.project.fieldTypes();
    for (const fieldType of fieldTypes) {
      if (fieldType.data) {
        const fieldTypeContent = createFieldTypeFacts(fieldType.data);
        setProgram(fieldType.data.name, fieldTypeContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setLinkTypesPrograms() {
    const linkTypes = this.project.linkTypes();
    for (const linkType of linkTypes) {
      if (linkType.data) {
        const linkTypeContent = createLinkTypeFacts(linkType.data);
        setProgram(linkType.data.name, linkTypeContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setWorkflowsPrograms() {
    const workflows = this.project.workflows();
    for (const workflow of workflows) {
      if (workflow.data) {
        const workflowContent = createWorkflowFacts(workflow.data);
        setProgram(workflow.data.name, workflowContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setReportsPrograms() {
    ...
    for (const report of reports) {
      if (report.data) {
        const reportContent = createReportFacts(report.data);
        setProgram(report.data.name, reportContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setTemplatesPrograms() {
    const templates = this.project.templates();
    for (const template of templates) {
      if (template.data) {
        const templateContent = createTemplateFacts(template.data);
        const cards = this.getCards(template.data.name);
        for (const card of cards) {
          const cardContent = await createCardFacts(card, this.project);
          setProgram(card.key, cardContent, [ALL_CATEGORY]);
        }
        setProgram(template.data.name, templateContent, [ALL_CATEGORY]);
      }
    }
  }
  ...
  private async setCalculationsPrograms() {
    const calculations = this.project.calculations(ResourcesFrom.all);
      ...
      try {
        if (calculation) {
          ...
          if (!resource.calculation) {
            this.logger.info(
              `Calculation resource '${calculation.data?.name}' does not have calculation file`,
            );
            continue;
          }
          setProgram(calculation.data!.name, resource.calculation, [
            ALL_CATEGORY,
          ]);
        }
      } catch (error) {
  ...
  public async resourceLogicProgram(
    ...
    const resource = this.project.resourceByType(resourceName).data;
    if (!resource) {
      throw new Error(
