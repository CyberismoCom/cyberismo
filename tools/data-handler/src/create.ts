...
export class Create extends EventEmitter {
  ...
  /**
   * Creates resource object from resource name.
   * @param project
   * @param name
   * @returns
   * @todo: Should this be part of Resource classes?
   */
  public static createResourceObject(project: Project, name: ResourceName) {
    if (name.type === 'cardTypes') {
      return new CardTypeResource(project, name);
    }
    if (name.type === 'fieldTypes') {
      return new FieldTypeResource(project, name);
    }
    if (name.type === 'linkTypes') {
      return new LinkTypeResource(project, name);
    }
    if (name.type === 'workflows') {
