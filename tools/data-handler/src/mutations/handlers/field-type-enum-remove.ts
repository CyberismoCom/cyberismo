...
/**
 * FieldTypeResource.update removes the value from the enum definition and
 * persists it (it does not cascade). The handler then owns the consumer-side
 * cascade: with a replacementValue, every card holding the removed value is
 * rewritten to the replacement; without one, the value is removed from the
 * enum definition only and cards keep their orphaned value (they are NOT
 * nulled).
 */
