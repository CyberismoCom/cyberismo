...
export class Validate {
  ...
  private async getAndCacheResource<Type>(
    ...
    let resource = cachedValues.get(valueName);
    if (!resource) {
      resource = await project.resource<Type>(valueName);
    }
    // not found from project, return undefined
    if (!resource) {
      return undefined;
    }
    // otherwise cache and return the resource
