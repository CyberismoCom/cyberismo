...
* {{#xref}}"cardKey":"docs_wp769ee4"{{/xref}}
* {{#xref}}"cardKey":"docs_4ha2rf9l"{{/xref}}
* {{#xref}}"cardKey":"docs_t5gkijm4"{{/xref}}
...
See {{#xref}}"cardKey":"docs_chclhxsk"{{/xref}} for a detailed description of how Cyberismo executes logic programs and Graphviz to produce a graph.
...
We can override the default card key by defining another card key in the parameters of the graph macro. For example, the following graph macro sets the card key to {{#xref}}"cardKey":"docs_13"{{/xref}}.
