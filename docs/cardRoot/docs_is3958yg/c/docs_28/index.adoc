...
The fields of cards can be of different data types, which are mapped in logic programs to strings, integers, and symbolic constants as follows. This mapping is used in `field` terms, which are discussed below. For a description of the data types of Cyberismo fields, see {{#xref}}"cardKey":"docs_25"{{/xref}}.
...
If you are using ASP outside the context of Cyberismo, you can use `#show` directives to select a subset of the answer set that you are interested in seeing. In Cyberismo, we use the built-in query language to define the results we are interested in; see {{#xref}}"cardKey":"docs_33"{{/xref}}.
...
Cyberismo logic programs can use the built-in external functions for working with dates and strings; see {{#xref}}"cardKey":"docs_fk6fimx4"{{/xref}}. External functions are similar to functions in mainstream programming languages, as they process their arguments and return a string or an integer.

== Naming conventions

Cyberismo uses a `camelCase` naming convention in logic programs. See {{#xref}}"cardKey":"docs_8"{{/xref}} for more information.
