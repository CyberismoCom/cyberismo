...
If your deployment uses a dedicated git-service container, set `GIT_SERVICE_URL` (for example `http://git-service:8080`). When set, Cyberismo delegates git clone and remote tag operations to git-service instead of making network git calls directly.
